...
class Colors:
	...
	def combine_colors(self, colors:list):
		"""
		Combines a list of `RGB` colors. 

		Parameters:
			colors:list - a list of `RGB` colors
		Returns:
			combinedColor - an `RGB` color object of the combined colors
		"""
		return Colors.RGB(sum([color._red for color in colors])//len(colors), sum([color._green for color in colors])//len(colors), sum([color._blue for color in colors])//len(colors))